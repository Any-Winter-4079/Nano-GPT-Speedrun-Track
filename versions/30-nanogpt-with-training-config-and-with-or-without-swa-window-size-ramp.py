...
#######################################################################################################
# step: 2,200 | train loss: 3.27450228 | train ppl: 26.43 | train step time: 178.79 ms |              #
# adamw lr: 0.00151495 | tok/s: 1,466,193.40 | total toks: 576,978,944 | total train time: 6.18 min | #
# sw size: 1024 | max q_scale raw/eff: 1.7708/1.7708 | max k_scale raw/eff: 1.7709/1.7709             #
# resetting val loader at step 2200                                                                   #
# new best val loss: 3.27396441                                                                       #
# step: 2,200 | val loss: 3.27396441 | val ppl: 26.42 | val time: 2,800.50 ms                         #
# val loss 3.27396441 reached target 3.28                                                             #
...
try:
    ...
    if not _ALREADY_REGISTERED:
        ...
        def _tanh_approx_lowering(inp: torch.Tensor) -> torch.Tensor:
            # inline ptx (f32)
...
@torch.compile
def zeropower_via_newtonschulz5(G: torch.Tensor, steps: int = 10, muon_eps: float = 1e-7) -> torch.Tensor:
    """
...
class DataLoader:
    ...
    def load_shard_tokens(self, shard_path: str) -> torch.Tensor:
    ...
    def next_batch(self) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
...
class MLP(nn.Module):
    def __init__(self, gpt_config: GPTConfig) -> None:
        ...
        if self.activation_name == "swiglu":
            ...
            self.hidden_dim = int(round((2.0/3.0) * gpt_config.up_proj_factor * gpt_config.d_model)) if gpt_config.use_fair_swiglu else gpt_config.up_proj_factor * gpt_config.d_model
...
class GPT(nn.Module):
    def __init__(self, gpt_config: GPTConfig, training_config: TrainingConfig) -> None:
        ...
        self.sliding_window_size_ramp_steps = gpt_config.sliding_window_size_ramp_steps
        ...
        if self.use_sliding_window_size_ramp:
            ...
            assert self.use_sliding_window_attention, "SWA window size ramp up requires SWA"
            ...
            initial_sliding_window_size = self.sliding_window_min_size
        else:
            initial_sliding_window_size = self.sliding_window_max_size
        # register as a 0-dim tensor buffer so torch.compile treats it as a dynamic tensor input
        self.register_buffer(
            "sliding_window_size",
            torch.tensor(initial_sliding_window_size, dtype=torch.int32),
...
def get_sliding_window_size(
    ...
    sliding_window_size_ramp_steps: int,
    flex_block_size: int,
    enforce_even_blocks: bool,
    ) -> int:
    if not use_sliding_window_size_ramp:
        raw_sliding_window_size = sliding_window_max_size
    else:
        t = min(1.0, step / sliding_window_size_ramp_steps)
        raw_sliding_window_size = int(round(sliding_window_min_size + t * (sliding_window_max_size - sliding_window_min_size)))

    # num_blocks_raw could be 0 is say sliding_window_min_size < flex_block_size (odd choice, but still)
    # Such a case would lead to a window size of int(0 * flex_block_size) = 0. This is handled:
    # by max(1, num_blocks_raw) if we enforce even blocks
    # by max(sliding_window_min_size, ...) if we do not
    num_blocks_raw = math.floor(raw_sliding_window_size / flex_block_size)

    if enforce_even_blocks:
        final_num_blocks = math.ceil(max(1, num_blocks_raw) / 2) * 2
        return int(final_num_blocks * flex_block_size)
    else:
        return max(sliding_window_min_size, int(num_blocks_raw * flex_block_size))
...
def save_checkpoint(
        step: int,
        torch_rng_state_cpu: torch.Tensor,
        torch_rng_state_cuda: torch.Tensor,
        val_loss: float,
        train_loss: float,
...
try:
    for step in range(start_step, max_train_steps):
        ...
        if raw_gpt_model.use_sliding_window_size_ramp:
            new_window_size_int = get_sliding_window_size(
                step=step,
                ...
                sliding_window_size_ramp_steps=raw_gpt_model.sliding_window_size_ramp_steps,
            ...
            raw_gpt_model.sliding_window_size.fill_(new_window_size_int)
