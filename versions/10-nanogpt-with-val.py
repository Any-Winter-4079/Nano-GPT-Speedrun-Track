...
# step: 9 | train loss: 10.38378906 | time: 1,237.34 ms | norm: 3.0186 | lr: 0.00000839 | tok/s: 423,722.19 | total toks: 5,242,880 | total time: 0.56 min
...
class DataLoader:
    ...
    def show_shard_info(self):
        ...
        print(f"per-gpu grad accumulation mini-steps for {self.split} shard {self.current_shard_idx} (each mini-step processing {self.grad_accum_tokens_per_mini_step:,} tokens): {(len(self.tokens) // self.grad_accum_tokens_per_mini_step):,}")
...
def sample(sample_sequences, max_new_tokens=5, temperature=1.0, top_k=None, top_p=None):
    ...
    with torch.no_grad():
        ...
        for i in range(len(sample_sequences)):
            ...
            print(f"[rank {ddp_rank}] seq {i} >>> {decoded}")
    gpt_model.train()
...
if master_process:
    print(f"per-gpu gradient accumulation mini-steps: {grad_accum_mini_steps}")
...
config_filename = os.path.join(log_dir, f"config_{timestamp}.txt")
...
for step in range(max_steps):
    ...
    if (step % sample_interval == 0 and step > 0) or step == max_steps - 1:
        ...
        sample(sample_sequences, max_new_tokens=max_new_tokens, top_k=50)
    ...
    if (step % val_interval == 0 and step > 0) or step == max_steps - 1:
        ...
        gpt_model.eval()
        if master_process:
            print(f"resetting val loader at step {step}")
        ...
        if master_process:
            val_log_content = f"step: {step:,} | val loss: {val_loss.item():.8f} | time: {1000*(val_step_t):,.2f} ms"
