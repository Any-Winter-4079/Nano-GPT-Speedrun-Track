...
optimizer = torch.optim.AdamW(gpt_model.parameters(), lr=max_lr, betas=betas, eps=eps)
...
for step in range(max_steps):
    ...
    end_t = time.time()
    print(f"step: {step:,} | grad norm: {norm:.4f} | lr: {lr:.6f} | tok/s: {batch_size * seq_len / (end_t - start_t):,.2f}")
